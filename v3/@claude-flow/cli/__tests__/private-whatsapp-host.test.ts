...
vi.mock('../src/mcp-tools/private-agent-settings.js',()=>({createPrivateAgentSettingsTools:()=>['agent_settings_patch_prepare','agent_settings_patch_apply'].map(name=>({name,description:'private',inputSchema:{type:'object'},cacheable:false,handler:privateCall}))}));
...
it('settings uses only hierarchical/settings reads and settings write, with fixed policy authority',async()=>{createProtectedWhatsAppHttpHost({},()=>({}),{port:8081,tools:['agent_settings_patch_prepare','agent_settings_patch_apply']});const tools=register.mock.calls[0][0];await tools[0].handler({});await tools[1].handler({namespace:'bypass'});expect(policy.mock.calls.map(c=>[c[1].namespace,c[3].actionType])).toEqual([['hierarchical:semantic','memory.read'],['ruclip-api-agent-settings','memory.read'],['hierarchical:semantic','memory.read'],['ruclip-api-agent-settings','memory.read'],['ruclip-api-agent-settings','memory.write']]);expect(privateCall).toHaveBeenCalledTimes(2);});
