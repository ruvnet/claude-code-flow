...
export function data() {
 ...
 const seal=raw=>{const req=JSON.parse(raw);const op={version:1,domain:'cognitum.protected-service-operation.v1',epoch:config.epoch,subject:config.subject,action:assertion.action,companyId:req.companyId,target:`ruclip:whatsapp-group-send-approval:${encodeURIComponent(req.companyId)}:${encodeURIComponent(req.groupId)}:${encodeURIComponent(req.assertion.reservationId)}`,bodySha256:createHash('sha256').update(raw).digest('hex'),nonce:'operation1',issuedAt:NOW-10,expiresAt:NOW+9000};const bytes=Buffer.from(JSON.stringify(op));return bytes.toString('hex')+'.'+sign(null,bytes,service.privateKey).toString('hex');};
