...
export function data() {
 ...
 const seal=raw=>{const req=JSON.parse(raw);const requestPhase=req.kind==='cognitum.whatsapp.approval-request.claim.v1'?'claim':req.kind==='cognitum.whatsapp.approval-request.published.v1'?'published':null;const op={version:1,domain:'cognitum.protected-service-operation.v1',epoch:config.epoch,subject:config.subject,action:requestPhase?`whatsapp.approval-request.${requestPhase}`:req.kind==='cognitum.whatsapp.consume.v1'?'whatsapp.send.consume':assertion.action,companyId:req.companyId,target:`ruclip:whatsapp-group-send-${requestPhase?'approval-request':'approval'}:${encodeURIComponent(req.companyId)}:${encodeURIComponent(req.groupId)}:${encodeURIComponent((req.assertion??req.intent).reservationId)}`,bodySha256:createHash('sha256').update(raw).digest('hex'),nonce:'operation1',issuedAt:NOW-10,expiresAt:NOW+9000};const bytes=Buffer.from(JSON.stringify(op));return bytes.toString('hex')+'.'+sign(null,bytes,service.privateKey).toString('hex');};
